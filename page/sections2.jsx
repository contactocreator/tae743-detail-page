...
const Clinical02Section = () => {
  ...
  return (
    ...
    >
      ...
      {/* 3개 카드 */}
      {/* 3개 카드 — React 애니메이션 (GIF로 교체 예정) */}
      <div data-anim="clinical02-bars" style={{ display: "flex", flexDirection: "column", gap: 25.35, paddingTop: 30.843 }}>
        {items.map((it, i) => {
          const num = Math.round(it.from + (it.to - it.from) * progress);
          const barWidth = it.barFrom + (it.barTo - it.barFrom) * progress;
          return (
            <div
              key={i}
              style={{
            ...
            >
              <div style={{ position: "relative", height: 117.539, width: "100%" }}>
                <div style={{ position: "absolute", top: 35.91, left: 0, display: "flex", flexDirection: "column", gap: 4.225 }}>
                  <div style={{ fontSize: 28.519, fontWeight: 700, color: "#14160f", lineHeight: "44.215px", whiteSpace: "nowrap" }}>
                    {it.label}
                  </div>
                  <div className="mono" style={{ fontSize: 20.069, fontWeight: 400, color: "#7a8074", letterSpacing: "2.0069px", lineHeight: "31.117px", whiteSpace: "nowrap" }}>
                    {it.sub}
                  </div>
                </div>
                <div className="mono" style={{ position: "absolute", top: 0, right: 0, fontSize: 59.15, fontWeight: 600, color: "#2d4739", letterSpacing: "-1.183px", lineHeight: "91.683px", textAlign: "right" }}>
                  {num}{it.suffix}
                </div>
              </div>
